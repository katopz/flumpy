...
package flump.export
{
	...
	public class ProjectController
	{
		...
		public function ProjectController()
		{
			...
			_importChooser = new DirChooser(null);
			_importChooser.changed.add(setImportDirectory);
			_exportChooser = new DirChooser(null);
		...
		public function initConfig(configFile:File = null):void
		{
			// config
			...
			if (_confFile != null)
			{
				try
				{
					...
					{
						_errorsGrid.dataProvider.addItem(new ParseError(_confFile.nativePath, ParseError.CRIT, "Import directory doesn't exist ('" + dir + "')"));
						_confFile = null;
						_conf = null;
					}
					else
				...
				catch (e:Error)
				{
					...
					_errorsGrid.dataProvider.addItem(new ParseError(_confFile.nativePath, ParseError.CRIT, "Unable to read configuration"));
					_confFile = null;
					_conf = null;
				}
			}
		...
		public function get win():Object
		{
			return _win;
		}
		...
		protected function setupMenus():void
		{
			...
			var fileMenuItem:NativeMenuItem = _win.nativeWindow.menu.addSubmenu(new NativeMenu(), "File");
			...
			var openMenuItem:NativeMenuItem = fileMenuItem.submenu.addItemAt(new NativeMenuItem("Open Project..."), 1);
			...
			const saveMenuItem:NativeMenuItem = fileMenuItem.submenu.addItemAt(new NativeMenuItem("Save Project"), 3);
			saveMenuItem.keyEquivalent = "s";
			saveMenuItem.addEventListener(Event.SELECT, F.callback(save));

			const saveAsMenuItem:NativeMenuItem = fileMenuItem.submenu.addItemAt(new NativeMenuItem("Save Project As..."), 4);
		...
		protected function saveConf(onSuccess:Function):void
		{
			Files.write(_confFile, function(out:IDataOutput):void
			{
				...
				if (_importChooser.dir != null)
				{
					...
					if (_conf.importDir == null)
						_conf.importDir = _importChooser.dir.nativePath;
				}
				...
				if (_exportChooser.dir != null)
				{
					...
					if (_conf.exportDir == null)
						_conf.exportDir = _exportChooser.dir.nativePath;
				}
		...
		{
			_win.export.enabled = _exportChooser.dir != null && _flashDocsGrid.selectionLength > 0 && _flashDocsGrid.selectedItems.some(function(status:DocStatus, ... _):Boolean
			{
		...
		{
			if (_exportChooser.dir == null || _conf.exports.length == 0)
				return null;
			...
			Files.list(base, exec).succeeded.add(function(files:Array):void
			{
				...
				for each (var file:File in files)
				{
					if (Files.hasExtension(file, "xfl"))
					{
						if (ignoreXflAtBase)
						{
							_errorsGrid.dataProvider.addItem(new ParseError(base.nativePath, ParseError.CRIT, "The import directory can't be an XFL directory, did you mean " + base.parent.nativePath + "?"));
						}
						else
		...
		protected var _flashDocsGrid:Object;
		protected var _errorsGrid:Object;
